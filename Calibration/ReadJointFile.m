...
% Direct usage example: data = ReadJointFile([rawDataPath '170328\'], '170328_073.dat')
...
%         data.profileY = fread(fid,1,'float64');
    ...
    elseif data.datatype == 3 %MCL raster scan   %AT: this is for movable mirror scans, on the old trap
        data.scandir = fread(fid,1,'float64');%1 = x scan, 0 = y scan
...
%     disp(size(D));
    ...
    if data.trpossaved == 1
        ...
        if data.oldtrap  
            ...
            elseif length(F) == length(data.A_X)*5
                F = (reshape(F,5,[]))';     
                data.trappos1 = F(:,1)';
                data.trappos2 = F(:,2)';
                data.offsetfitX = F(:,3)';
        ...
        %%
        fln = [startpath rootfile(1:(end-4)) '_lbvwoffsetfit.dat'];
        fn = fullfile(fln);
        
        if exist(fn,'file') == 2
            fid = fopen([startpath rootfile(1:(end-4)) '_lbvwoffsetfit.dat'],'r','ieee-be');
