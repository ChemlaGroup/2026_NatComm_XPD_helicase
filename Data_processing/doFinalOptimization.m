...
%             params = getMLestimate(curvefun,'off',p0,X,z,lb,ub,mask); % 190523 SY above is original 
            params = getMLestimate(curvefun,'off',p0,X,z,lb,ub,mask); % 230910: turning output off again (AVT)
        case 'bootstrap'
    ...
    results.ssep = sum((abs(z)-abs(results.G)).^2);
